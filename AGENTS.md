...
- This file is loaded for work in this repository; the user does not need to repeat these rules in each prompt. For a narrow change, inspect only the relevant files. Read `docs/PROJECT_CONTEXT.md` for broad or architectural work, `docs/SETUP.md` for setup/deployment, and the SQL schema/migrations when changing Supabase.
